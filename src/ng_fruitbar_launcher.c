...
static void      _item_new           (Ngi_Box *box, Efreet_Desktop *dekstop, int instant, Ngi_Item_Fruitbar *after);
static void      _item_fill          (Ngi_Item_Fruitbar *it);

static void      ngi_fruitbar_launcher_cb_drop_enter      (void *data, const char *type, void *event_info);
static void      ngi_fruitbar_launcher_cb_drop_leave      (void *data, const char *type, void *event_info);
static void      ngi_fruitbar_launcher_cb_drop_move       (void *data, const char *type, void *event_info);
static void      ngi_fruitbar_launcher_cb_drop_end        (void *data, const char *type, void *event_info);
static Eina_Bool  ngi_fruitbar_launcher_cb_icons_update(void *data, int type, void *event);


static void
_ngi_fruitbar_launcher_add_dnd_event_handlers(Ngi_Box *box)
{
   const char *drop[] =
   ...
   box->drop_handler = e_drop_handler_add
      (box->ng->win->drop_win, box,
       ngi_fruitbar_launcher_cb_drop_enter, ngi_fruitbar_launcher_cb_drop_move,
       ngi_fruitbar_launcher_cb_drop_leave, ngi_fruitbar_launcher_cb_drop_end,
       drop, 3, 0, 0, 0, 0);
}

static void
_ngi_fruitbar_launcher_add_efreet_event_handlers(Ngi_Box *box)
{
   char buf[4096];
   ...
   h = ecore_event_handler_add(E_EVENT_CONFIG_ICON_THEME, ngi_fruitbar_launcher_cb_icons_update, box);
   if (h) box->handlers = eina_list_append(box->handlers, h);
   h = ecore_event_handler_add(EFREET_EVENT_ICON_CACHE_UPDATE, ngi_fruitbar_launcher_cb_icons_update, box);
   if (h) box->handlers = eina_list_append(box->handlers, h);
   h = ecore_event_handler_add(EFREET_EVENT_DESKTOP_CACHE_UPDATE, ngi_fruitbar_launcher_cb_icons_update, box);
   if (h) box->handlers = eina_list_append(box->handlers, h);

   //Add callback for changes in the .order (listed applications) file
...
void
ngi_fruitbar_launcher_box_fill(Ngi_Box *box)
{
   _box_fill(box);
}

void
ngi_fruitbar_launcher_new(Ng *ng, Config_Box *cfg)
{
...
void
ngi_fruitbar_launcher_remove(Ngi_Box *box)
{
   Ecore_Event_Handler *h;
...
static Eina_Bool
ngi_fruitbar_launcher_cb_icons_update(void *data, int type, void *event)
{
...
_drop_item_new(Ngi_Box *box)
{
   ...
   it = (Ngi_Item*)E_NEW(Ngi_Item_Fruitbar, 1);
   it->box = box;
   ngi_item_show(it, 0);
...
static void
ngi_fruitbar_launcher_cb_drop_enter(void *data, const char *type, void *event_info)
{
...
static void
ngi_fruitbar_launcher_cb_drop_move(void *data, const char *type, void *event_info)
{
...
static void
ngi_fruitbar_launcher_cb_drop_leave(void *data, const char *type, void *event_info)
{
   Ngi_Box *box = (Ngi_Box *)data;
...
static void
ngi_fruitbar_launcher_cb_drop_end(void *data, const char *type, void *event_info)
{
   E_Event_Dnd_Drop *ev = (E_Event_Dnd_Drop *)event_info;
   Ngi_Box *box = data;
   Efreet_Desktop *app = NULL;
   Ngi_Item_Fruitbar *it = NULL;
...
static void
_item_new(Ngi_Box *box, Efreet_Desktop *desktop, int instant, Ngi_Item_Fruitbar *after)
{
   Ngi_Item_Fruitbar *it;
   ...
   fprintf(stderr, "Engage_Fruitbar: Added efreet desktop file %s.\n", desktop->name);

   ngi_item_show((Ngi_Item*)it, instant);
}

void
item_ngi_fruitbar_launcher_cb_free(Ngi_Item *item)
{
...
static void
_item_fill(Ngi_Item_Fruitbar *it)
{
   Evas_Object *o;
...
static void
ngi_fruitbar_launcher_cb_drag_del(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
   Ngi_Box *box = data;
...
static void
ngi_fruitbar_launcher_cb_drag_finished(E_Drag *d, int dropped)
{
   efreet_desktop_unref((Efreet_Desktop*)d->data);
}

void
item_ngi_fruitbar_launcher_cb_drag_start(Ngi_Item *item)
{
   Ngi_Item_Fruitbar *it = (Ngi_Item_Fruitbar*) item;
   E_Drag *d;
   Evas_Object *o;
   ...
   d = e_drag_new(box->ng->zone->container, x, y, drag_types, 1,
                  it->app, -1, NULL,
		  ngi_fruitbar_launcher_cb_drag_finished);
   efreet_desktop_ref(it->app);

   o = e_util_desktop_icon_add(it->app, MIN(w, h), e_drag_evas_get(d));
   e_drag_object_set(d, o);
   e_drag_resize(d, w, h);
   evas_object_event_callback_add(o, EVAS_CALLBACK_DEL, ngi_fruitbar_launcher_cb_drag_del, box);
...
void
item_ngi_fruitbar_launcher_cb_mouse_down(Ngi_Item *item, Ecore_Event_Mouse_Button *ev)
{
   Ngi_Item_Fruitbar *it = (Ngi_Item_Fruitbar *) item;
   Evas_Coord x, y, w, h;
   E_Menu *m;
...
void
item_ngi_fruitbar_launcher_cb_mouse_up(Ngi_Item *item, Ecore_Event_Mouse_Button *ev)
{
   Ngi_Item_Fruitbar *it = (Ngi_Item_Fruitbar *) item;
